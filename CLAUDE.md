...
docs/
  ...
  study-design.md       # RQs, conditions, the 14 items + acceptance rule, measures, rubric, consent
src/
  ...
  tasks.py              # the 14 items on 5 chart types, forms A and B. NO answer key — ships
...
- **Every item must pass the acceptance rule in `study-design.md` §4** (2026-09-22, extended
  2026-09-23 to the seven item types), and `analysis/keys.py` refuses one that fails.
  ...
  - A crossing (T7) happens once, the lines part by 5 points on each side, they meet at least a
...
- **RQ1 accuracy is T1–T7** (2026-09-23; T6 was scored separately while it was a gap item). Per-item
  and per-chart-type results are descriptive only: each chart type carries one item per form, the
  line chart three. T7, the crossing item, also gets adjacent-band credit as a pre-registered
  ...
  order; no answer position holds more than a third of the fourteen keys.
...
- 2026-09-23 — **T7 added** (uncommitted at time of writing): a crossing item, so one question asks
...
- **Still to do before the pilot:**
  ...
  - Set `consent.APPROVED = True` only once HSRRC approves, and re-pin the text hash if the wording
    changed.
- Next after IRB: pilot, and the checks in `study-design.md` §10: static accuracy on T3, T5 and T6
  (margins sit at the floor), T1, B-T4 and T7 form equivalence, T7 strict against adjacent-band,
...
- 2026-09-23 — **T7, a crossing item, added** after the bank was replaced: the six items asked
