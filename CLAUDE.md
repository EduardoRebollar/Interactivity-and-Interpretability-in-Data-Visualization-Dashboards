...
and `sort_change` from the bar chart's sort. No new event, no new key, no table change, so
...
- **RQ1 accuracy is T1–T6** (2026-09-23; T6 was scored separately while it was a gap item). Per-item
  and per-chart-type results are descriptive only: each chart type carries one item per form.
  Options that are countries are alphabetical, like the chart's entities; years run in calendar
  order; no answer position holds more than three of the twelve keys.
...
- Next after IRB: pilot, and the checks in `study-design.md` §10: static accuracy on T3, T5 and T6
  (margins sit at the floor), T1 and B-T4 form equivalence, and whether the bar sort and the map's
  coverage range are found at all.
