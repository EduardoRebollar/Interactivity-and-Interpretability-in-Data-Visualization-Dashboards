...
Per condition: accuracy (the RQ1 primary outcome over T1-T5, as the mean of per-participant
proportions, a skip scored incorrect), the pre-registered secondaries (skips excluded; adjacent-band
credit on the crossing items), skip counts, T6 on its own line, Paas mental effort (RQ3), the Likert
survey, and time on task. Plus the exclusion table section 7 requires. Inferential tests are
deliberately absent: they belong in the analysis notebook, run once, on the frames this package
produces.
"""
...
from analysis.reshape import SEPARATELY_REPORTED


def _rq1(tasks: pd.DataFrame) -> pd.Series:
    """Rows that belong in the RQ1 accuracy score: usable, and not T6 (section 7)."""
    return tasks["use_accuracy"] & ~tasks["task_id"].isin(SEPARATELY_REPORTED)


def accuracy(tasks: pd.DataFrame) -> pd.DataFrame:
    """Proportion correct over T1-T5 per participant per condition, summarised by condition."""
    usable = tasks[_rq1(tasks)]
    per_participant = (
...
def accuracy_answered(tasks: pd.DataFrame) -> pd.DataFrame:
    """Secondary: proportion correct among ANSWERED RQ1 items only, skips excluded (section 7)."""
    usable = tasks[_rq1(tasks) & ~tasks["skipped_answer"].astype(bool)]
    per_participant = (
        usable.groupby(["condition", "participant_id"])["correct"].mean().rename("prop_correct")
    )
    return per_participant.groupby("condition").agg(["count", "mean", "std"])
...
def crossing_adjacent(tasks: pd.DataFrame) -> pd.DataFrame:
    """Secondary: the crossing items (T4, T5) under strict and adjacent-band scoring."""
    crossings = tasks[tasks["use_accuracy"] & (tasks["kind"] == "crossing")]
    return crossings.groupby(["condition", "task_id"])[["correct", "correct_adjacent"]].mean()


def gap_check(tasks: pd.DataFrame) -> pd.DataFrame:
    """T6, reported on its own and never in the RQ1 score: its answer is on screen (section 4)."""
    separate = tasks[tasks["use_accuracy"] & tasks["task_id"].isin(SEPARATELY_REPORTED)]
    return separate.groupby("condition")["correct"].agg(["count", "mean"])
...
def render(tasks: pd.DataFrame, conditions: pd.DataFrame, exclusions: list[Exclusion]) -> str:
    ...
    sections = [
        ...
        ("Accuracy, RQ1 primary: proportion correct per participant, T1-T5", accuracy(tasks)),
        ...
        ("Crossing items secondary: strict vs adjacent-band credit", crossing_adjacent(tasks)),
        ("T6 gap check, reported separately: proportion correct", gap_check(tasks)),
